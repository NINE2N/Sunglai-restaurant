...
app.get('/menu', async (req, res) => {
    try {
        const [products] = await dbConnection.execute(`
            ...
            p.pro_price AS price, pt.pt_name_eng AS category, pt.pt_name_th AS categoryTh
            FROM products p
        ...
        const groupedProducts = products.reduce((acc, product) => {
            ...
            if (!acc[categoryKey]) {
                acc[categoryKey] = {
                    ...
                    categoryNameTh: product.categoryTh,
                    products: []
                };
            }
        ...
        res.render('menu', { groupedProducts });
    } catch (error) {
...
app.post('/signup', async (req, res) => {
    ...
    const { username, password, tel, title_name, name, surname, gender } = req.body;
    // console.log({ username, password, tel, title_name, name, surname, gender });
    ...
    try {
        ...
        if (existingUser.length > 0) {
            ...
            await dbConnection.execute(
                "INSERT INTO `user`(`log_id`, `title_id`, `u_name`, `u_surname`, `gender_id`, `u_tel`, `u_date`) VALUES (?, ?, ?, ?, ?, ?, NOW())",
                [logId, title_name, name, surname, gender, tel]
            );
...
app.get('/shop', async (req, res) => {
    ...
    try {
        ...
        res.render('shop', { products, promotions, user: user[0] });
    } catch (error) {
        console.error('Error fetching products or user:', error);
        res.status(500).send('Server Error');
    }
...
app.post('/add-product', upload.single('img'), async (req, res) => {
    ...
    try {
        ...
        res.redirect('/shop'); // หลังจากเพิ่มเสร็จให้เปลี่ยนไปที่หน้าร้านค้า
    } catch (error) {
...
app.post('/update-product/:id', upload.single('img'), async (req, res) => {
    ...
    try {
        ...
        res.redirect('/shop'); // หลังจากอัปเดตเสร็จ เปลี่ยนไปที่หน้าร้านค้า
    } catch (error) {
...
app.get('/user_manage', async (req, res) => {
    ...
    try {
        // ดึงข้อมูลผู้ใช้
        const [user] = await dbConnection.execute(`
        SELECT u_id,title_name,u_name,u_surname,gender_name,u_tel,u_point 
...
app.get('/user_edit/:id', async (req, res) => {
    ...
    try {
        // ดึงข้อมูลสินค้าจากฐานข้อมูลโดยใช้ user_id
        const [user_edit] = await dbConnection.execute(`
            SELECT u_id,user.title_id AS title_id,u_name,u_surname,user.gender_id AS gender_id,u_tel,u_point 
            FROM user LEFT JOIN titlename ON user.title_id = titlename.title_id 
        ...
        // ส่ง user[0] ไปยังหน้า user_edit
...
app.post('/update-user/:id',  async (req, res) => {
    ...
    try {
            // อัปเดตเฉพาะข้อมูลอื่นๆ ยกเว้นรูปภาพ
            await dbConnection.execute(
                'UPDATE `user` SET `title_id`=?,`u_name`=?,`u_surname`=?,`gender_id`=?,`u_tel`=? WHERE u_id = ?',
...
app.post('/confirm-order', async (req, res) => {
    ...
    try {
        ...
        const [orderResult] = await dbConnection.execute(
            'INSERT INTO foodorder (u_id, total_price,promo_id, or_date, status_id) VALUES (?, ?, ?, NOW(),1)',
...
app.post('/update-order-status', async (req, res) => {
    ...
    try {
        // อัพเดทสถานะของออเดอร์ในตาราง foodorder โดยใช้ status_id
        await dbConnection.execute(
            'UPDATE foodorder SET status_id = ? WHERE or_id = ?',
            [statusId, orderId]
        );
...
app.get('/orders/:userId', async (req, res) => {
    ...
    try {
        const [orders] = await dbConnection.execute(`
            SELECT 
                foodorder.or_id AS order_id,
                status.status_name AS status,
                foodorder.total_price,
                foodorder.promo_id,  -- ส่วนลดที่ใช้
            ...
            FROM foodorder
            LEFT JOIN order_item ON foodorder.or_id = order_item.or_id
            LEFT JOIN products ON order_item.pro_id = products.pro_id
            LEFT JOIN status ON foodorder.status_id = status.status_id
            LEFT JOIN promotion ON foodorder.promo_id = promotion.promo_id  -- เชื่อมตาราง promotion
            WHERE foodorder.u_id = ?
            ORDER BY foodorder.or_id DESC
        `, [userId]);
...
app.post('/update-order-status', async (req, res) => {
    ...
    try {
        await dbConnection.execute(
            'UPDATE foodorder SET status = ? WHERE or_id = ?',
            [status, orderId]
        );
    ...
    try {
        ...
        const [orders] = await dbConnection.execute(`
            SELECT 
                ...
                foodorder.or_date AS order_date,
                foodorder.total_price AS total_amount
            FROM 
                foodorder
            JOIN 
                order_item ON foodorder.or_id = order_item.or_id
            JOIN 
                user ON foodorder.u_id = user.u_id
            JOIN 
...
io.on('connection', (socket) => {
    ...
    async function fetchGroupedOrders() {
        const [orders] = await dbConnection.execute(`
            ...
            LEFT JOIN order_item ON foodorder.or_id = order_item.or_id 
            LEFT JOIN products ON order_item.pro_id = products.pro_id 
            LEFT JOIN promotion ON foodorder.promo_id = promotion.promo_id
            LEFT JOIN status ON foodorder.status_id = status.status_id;
        `);
    ...
    socket.on('updateStatus', async ({ orderId, statusId }) => {
        try {
            await dbConnection.execute(`UPDATE foodorder SET status_id = ? WHERE or_id = ?`, [statusId, orderId]);
